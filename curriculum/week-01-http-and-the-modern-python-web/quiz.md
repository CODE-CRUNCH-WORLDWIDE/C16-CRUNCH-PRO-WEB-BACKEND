...
**C** — `Host` is the only required HTTP/1.1 request header. Without it, the server can't disambiguate virtual hosts and will typically return `400`.
...
**C** — PEP 3333 defines a WSGI app as a callable with `(environ, start_response)` returning an iterable of bytes.
...
**B** — `PUT` replaces the entire resource. `PATCH` partially updates. `POST` creates or runs an action; not for replacement.
...
**A** — `401` is for "no/invalid authentication credentials." `403` is "authenticated but not allowed."
...
**B** — WSGI is sync-only and cannot support WebSockets. ASGI was designed to handle them and added the event-based protocol model.
...
**C** — FastAPI is purpose-built for this case: typed, OpenAPI-first, async, Pydantic validation.
...
**B** — `time.sleep` is a synchronous blocking call. It freezes the event loop. Use `await asyncio.sleep(2)` instead.
...
**C** — `404` for "resource doesn't exist." `400` is for malformed requests; `500` is for server errors.
...
**B** — `Host` is required so servers can route between multiple sites hosted on the same IP. Modern HTTPS uses SNI (Server Name Indication) in the TLS handshake for similar reasons.
